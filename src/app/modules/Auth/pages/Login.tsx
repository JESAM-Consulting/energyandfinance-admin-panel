...
export default function Login() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    // setLoader(true);
    e.preventDefault();
    if (!loginData.adminEmail && !loginData.password) {
      setLoading(false)
      setErrors({
        adminEmail: "E-Mail ist erforderlich*",
        password: "Passwort wird benötigt*",
      });
    } else if (loginData.adminEmail === "" && loginData.password === "") {
      setErrors({ ...errors, adminEmail: "E-Mail ist erforderlich*" });
      setLoading(false)
    } else if (!loginData.adminEmail || loginData.adminEmail === "") {
      setErrors({ ...errors, adminEmail: "E-Mail ist erforderlich*" });
      setLoading(false)
    } else if (!loginData.adminEmail || regexEmail.test(loginData.adminEmail) === false) {
      setErrors({ ...errors, adminEmail: "E-Mail ist ungültig*" });
      setLoading(false)
    } else if (!loginData.password || loginData.password === "") {
      setErrors({ ...errors, password: "Passwort wird benötigt*" });
      setLoading(false)
    } else {
      loginData.adminEmail = loginData.adminEmail.toLowerCase();
    ...
    <div className="login-form login-signin" id="kt_login_signin_form">
      ...
      <form onSubmit={(e) => handleKeyPress(e)}>
        <div className="form-group fv-plugins-icon-container" >
          ...
          />
          <span className="text-danger">{errors.adminEmail}</span>
        </div>
        ...
        <div className="form-group d-flex flex-wrap justify-content-center align-items-center">
          <button
            ...
            style={{backgroundColor:"#93c45e",color:"white"}}
            onClick={(e) => {
              handleSubmit(e);
            }}
